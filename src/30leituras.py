...
dataframe = pd.read_csv('data/apenas_leituras_30.csv', usecols=cols, engine= "python")
...
model = Sequential()
model.add(Dense(units=30, input_shape=(look_back,),activation='relu'))
model.add(Dense(2))
model.add(Dense(12))

model.compile(loss="mean_squared_error", optimizer="adam")

model.fit(trainX, trainY, epochs=12, batch_size = 1, verbose = 1, validation_data=(validationX, validationY))

trainScore = model.evaluate(trainX, trainY, verbose=0)
print('Train Score: %.2f MSE (%.2f RMSE)' % (trainScore, math.sqrt(trainScore)))
testScore = model.evaluate(testX, testY, verbose=0)
print('Test Score: %.2f MSE (%.2f RMSE)' % (testScore, math.sqrt(testScore)))
model.save("models/modelo30.keras")
